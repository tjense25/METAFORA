...
findVariableSegments <- function(seg_betas, plot_file=NULL) {
   ...
   var_segs <- seg_cv2$CV2 > cv2.fit$trend(seg_cv2$mean) + 25*cv2.fit$trend(seg_cv2$mean)*cv2.fit$std.dev/sqrt(ncol(beta.mat))
   head(var_segs)
   table(var_segs)
...
fwrite(segment_betas, file=argv$varsegment_bed_out, quote=F,row.names=F,col.names=T,sep="\t")
