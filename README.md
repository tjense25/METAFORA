...
Metafora is written as a snakemake workflow wrapper around a series of R scripts. To run, clone this github repository, enter the directory, and then a user updates a config file to specify an input table of samples / methylation filepaths, reference genome, runtime parameters, and optional covariates file. Then the workflow can be run using the following snakemake command
```
