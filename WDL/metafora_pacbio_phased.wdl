...
workflow metafora {
    ...
    scatter (chromosome in autosomes) {
        call CreateTissueSampleReference as AutosomeCreateTissueSampleReference {
            input:
                meth_beds = FormatPacBioCombined.meth_bed,
                meth_beds_index = FormatPacBioCombined.meth_bed_tbi,
                cpg_bed = CreateCpgReference.cpg_bed,
                chr = chromosome,
                tissue = tissue,
                rscript = create_tissue_reference_script,
                min_segment_size = 20
        }
        ChromosomeReference autosome_reference = {
            "chromosome" : AutosomeCreateTissueSampleReference.chromosome,
            "beta_matrix" : AutosomeCreateTissueSampleReference.beta_matrix,
            "beta_matrix_tbi" : AutosomeCreateTissueSampleReference.beta_matrix_tbi,
            "depth_matrix" : AutosomeCreateTissueSampleReference.depth_matrix,
            "depth_matrix_tbi" : AutosomeCreateTissueSampleReference.depth_matrix_tbi
        }
    }

    scatter (chromosome in sex_chromosomes) {
        call CreateTissueSampleReference as SexChromosomeCreateTissueSampleReference {
            input:
                meth_beds = FormatPacBioCombined.meth_bed,
                meth_beds_index = FormatPacBioCombined.meth_bed_tbi,
                cpg_bed = CreateCpgReference.cpg_bed,
        ...
        ChromosomeReference sex_reference = {
                "chromosome" : SexChromosomeCreateTissueSampleReference.chromosome,
                "beta_matrix" : SexChromosomeCreateTissueSampleReference.beta_matrix,
                "beta_matrix_tbi" : SexChromosomeCreateTissueSampleReference.beta_matrix_tbi,
                "depth_matrix" : SexChromosomeCreateTissueSampleReference.depth_matrix,
                "depth_matrix_tbi" : SexChromosomeCreateTissueSampleReference.depth_matrix_tbi
            } 
    }
    
    call ComputeHiddenFactors {
            input:
                rscript = compute_hidden_factors_script,
                segment_betas = AutosomeCreateTissueSampleReference.seg_beta,
                segment_depths = AutosomeCreateTissueSampleReference.seg_depth,
                sex_segment_betas = SexChromosomeCreateTissueSampleReference.seg_beta,
                sex_segment_depths = SexChromosomeCreateTissueSampleReference.seg_depth,
    ...
    scatter (sample in ParseSampleTable.samples) {
        ...
        scatter (chromosome_reference in autosome_reference) {
                call CallOutliers {
                    input:
                    rscript = call_outliers_script,
                    sample = sample.name,
                    tissue = tissue,
                    chrom = chromosome_reference.chromosome,
                    beta_matrix = chromosome_reference.beta_matrix,
                    beta_matrix_tbi = chromosome_reference.beta_matrix_tbi,
                    depth_matrix = chromosome_reference.depth_matrix,
                    depth_matrix_tbi = chromosome_reference.depth_matrix_tbi,
                    global_pcs = ComputeHiddenFactors.global_pcs,
                    max_depth = 30,
                    min_segment_size = 20,
                    min_abs_zscore = 3,
                    min_abs_delta = 0.25
        ...
        scatter (chromosome_reference in sex_reference) {
                call CallOutliersSexChromosomes {
                    input:
                    rscript = call_sex_outliers_script,
                    sample = sample.name,
                    tissue = tissue,
                    chrom = chromosome_reference.chromosome,
                    beta_matrix = chromosome_reference.beta_matrix,
                    ...
                    depth_matrix_tbi = chromosome_reference.depth_matrix_tbi,
                    global_pcs = ComputeHiddenFactors.global_pcs,
                    sex_chrom_df = ComputeHiddenFactors.sex_summary_table,
                    max_depth = 30,
                    min_segment_size = 20,
                    min_abs_zscore = 3,
                    min_abs_delta = 0.25
        ...
        call CombineChromOutliers {
                input:
                    sample = sample.name,
                    tissue = tissue,
                    beds = flatten([CallOutliers.outlier_bed, CallOutliersSexChromosomes.outlier_bed]),
                    mats = flatten([CallOutliers.outlier_z_mat, CallOutliersSexChromosomes.outlier_z_mat])
            }

        call AnnotateHaplotypeDelta {
                input:
                    rscript = annotate_haplotype_delta_script,
                    sample = sample.name,
                    tissue = sample.tissue,
                    outlier_bed = CombineChromOutliers.outlier_bed,
                    combined_bed = FormatPacBioCombined.meth_bed,
                    combined_bed_tbi = FormatPacBioCombined.meth_bed_tbi,
                    hap1_bed = FormatPacBioHapH1.meth_bed,
                    hap1_bed_tbi = FormatPacBioHapH1.meth_bed_tbi,
                    hap2_bed = FormatPacBioHapH1.meth_bed,
                    hap2_bed_tbi = FormatPacBioHapH2.meth_bed_tbi
            }

        }

        call CombineOutliers {
                input:
                    tissue = tissue,
                    beds = CombineChromOutliers.outlier_bed
            }

        #call SummaryPlots {
        #        input:
        #            rscript = plot_summary_figures_script,
        #            outlier_bed = CombineOutliers.combined_outliers,
        #            covariates = ComputeHiddenFactors.global_pcs,
        #            min_abs_zscore = 3,
        #            min_abs_delta = 0.25
        #    }
    ...
    output {
        File cpg_bed = CreateCpgReference.cpg_bed
    }
}
